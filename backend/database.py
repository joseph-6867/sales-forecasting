...
def db_upsert_user(user_id, email, full_name="", role="analyst", avatar_url="", password_hash=None):
    ...
    # Raises on failure so the caller (_upsert_user_record) can log the real error
...
def db_create_project(user_id, name, description=""):
    ...
    try:
        ...
        import traceback
        print(f"[DB] create_project error for user_id={user_id!r}: {e}")
        traceback.print_exc()
        return None
...
def db_delete_project(project_id, user_id):
    """Soft delete — sets status = 'deleted'. Project stays in DB."""
    db = get_db()
    try:
...
def db_hard_delete_project(project_id, user_id):
    """
    Hard delete — permanently removes the project row and all child rows
    (datasets, forecasts, model_metrics, reports) via ON DELETE CASCADE.
    """
    db = get_db()
    try:
        db.table("projects")\
