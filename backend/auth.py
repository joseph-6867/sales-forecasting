...
def auth_google_login_url() -> str:
    ...
    BUG 1 FIX: Removed `query_params` (access_type/prompt) from the OAuth options.
    Those params forced a PKCE flow where the code verifier is generated by
    Supabase JS on the client side — but Python has no access to that verifier,
    so the code exchange always failed. Removing them lets Supabase handle
    the full PKCE flow automatically and correctly.
    ...
    # ✅ BUG 1 FIX: No query_params here — they broke PKCE code exchange.
    # Supabase handles PKCE automatically when left alone.
    res = supabase.auth.sign_in_with_oauth({
        "provider": "google",
        "options": {
            "redirect_to": site_url,
            "scopes":      "email profile",
            # ❌ REMOVED: "query_params": {"access_type": "offline", "prompt": "consent"}
            # These forced PKCE params that Python cannot satisfy (no client-side verifier).
        },
    })
