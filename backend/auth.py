...
def _upsert_user_record(user_id, email, full_name, role, avatar_url=""):
    """
    Sync the Supabase Auth user into public.users so that the FK on
    projects / datasets / forecasts / reports is satisfied.
    Called after EVERY successful login (email or Google). Non-fatal.
    """
    try:
        from backend.database import db_upsert_user
        db_upsert_user(
            user_id=user_id,
            email=email,
            full_name=full_name,
...
def _set_session_from_supabase(user, session):
    ...
    # Sync into public.users — required so projects FK works
    _upsert_user_record(user.id, user.email, full_name, role, avatar)
...
def auth_handle_google_callback() -> bool:
    ...
    frag_params = dict(urllib.parse.parse_qsl(fragment.lstrip("#")))
    access_token = frag_params.get("access_token")
    if not access_token:
        return False
